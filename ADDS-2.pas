procedure aDDS(K,l,v:integer); { http://forum.enjoysudoku.com/distributed-disjoint-subsets-t5423.html}
type
...
z2:=usenum2[w];	  
if {(size+1 = w) and }(count = popcnt(dword(usenum2[w] ))) then 
...
if  (count-1 = popcnt(dword(usenum2[w]-z2 ))) and (popcnt(dword(z2))=1)	
then
begin		   
 ...
 then
  begin
   ...
   for b:= 1 to w do 
	...
	if ((h[w] < 0) and (w >0)) or (count = popcnt(dword(usenum2[w] )))
      then
        begin	
